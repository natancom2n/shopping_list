defmodule ShoppingList.ListsTest do
  ...
  describe "lists" do
    ...
    import ShoppingList.ListsFixtures

    @invalid_attrs %{completed: nil, price: nil, product: nil}
    ...
    test "create_list/1 with valid data creates a list" do
      valid_attrs = %{completed: true, price: 120.5, product: "some product"}

      assert {:ok, %List{} = list} = Lists.create_list(valid_attrs)
      assert list.completed == true
      assert list.price == 120.5
      assert list.product == "some product"
    end
    ...
    test "update_list/2 with valid data updates the list" do
      list = list_fixture()
      update_attrs = %{completed: false, price: 456.7, product: "some updated product"}

      assert {:ok, %List{} = list} = Lists.update_list(list, update_attrs)
      assert list.completed == false
      assert list.price == 456.7
      assert list.product == "some updated product"
    end
